...
import { fetchBookingDetail, cancelBooking, clearCurrentBooking } from '../../store/bookingSlice.js';
...
const BookingDetailPage = () => {
  ...
  useEffect(() => {
    ...
    api.get(`/reviews/provider/${currentBooking.provider._id}`)
      .then(({ data }) => {
  ...
  const handleReviewSubmit = (e) => {
    ...
    api.post('/reviews', { bookingId: id, rating, comment })
      .then(() => {
  ...
  const { status, scheduledDate, scheduledTime, address, totalAmount, notes, cancellationReason, statusHistory, service, provider } = currentBooking;
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight">Booking Details</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <Card hoverEffect={false} className="p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-zinc-200">{service?.title}</span>
                <p className="text-xs text-zinc-400 leading-relaxed">{service?.description}</p>
              </div>
            </Card>

            <Card hoverEffect={false} className="p-6 flex flex-col gap-4">
              <h3 className="font-bold text-lg border-b border-zinc-800 pb-2">Schedule & Location</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div className="flex items-center gap-3 text-zinc-300">
                  <Calendar size={16} className="text-zinc-500" />
                  <span>{scheduledDate ? format(new Date(scheduledDate), 'PPP') : 'N/A'}</span>
                </div>
                <div className="flex items-center gap-3 text-zinc-300">
            ...
            <Card hoverEffect={false} className="p-6 flex flex-col gap-4">
              <h3 className="font-bold text-lg border-b border-zinc-800 pb-2">Booking Status History</h3>
              <div className="flex flex-col gap-4 relative pl-4 border-l border-zinc-800">
                {statusHistory?.map((hist, idx) => (
                  <div key={idx} className="relative flex flex-col gap-1">
                    ...
                    <span className="text-xs font-bold text-zinc-300 capitalize">{hist.status.replace('_', ' ')}</span>
                    <span className="text-[10px] text-zinc-500">
                      {format(new Date(hist.timestamp), 'PPp')}
                    </span>
                    {hist.note && <p className="text-xs text-zinc-400 bg-zinc-900/50 p-2.5 rounded border border-zinc-800/80 mt-1">{hist.note}</p>}
                  </div>
                ))}
              </div>
            </Card>
            ...
            {/* Leave Review Form */}
            {status === 'completed' && !hasReviewed && user?.role === 'customer' && (
              <Card hoverEffect={false} className="p-6 border border-indigo-500/20 bg-indigo-500/5 flex flex-col gap-4">
                <h3 className="font-bold text-lg text-indigo-400">Share Your Feedback</h3>
                <p className="text-xs text-zinc-400">Rate your experience with {provider?.user?.name || 'Local Expert'}</p>
                <form onSubmit={handleReviewSubmit} className="flex flex-col gap-4">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Rating</span>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Comments</label>
                    <textarea
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <Card hoverEffect={false} className="p-6 flex flex-col gap-4">
              <h3 className="font-bold border-b border-zinc-800 pb-2">Financials</h3>
              <div className="flex justify-between items-center text-sm">
                <span className="text-zinc-500">Service Fee</span>
                <span className="font-semibold text-zinc-300">₹{totalAmount}</span>
              </div>
              <div className="flex justify-between items-center text-sm border-t border-zinc-800/80 pt-3">
                ...
                <span className="text-lg font-bold text-indigo-400">₹{totalAmount}</span>
              </div>

              {/* Action Buttons */}
              {['pending', 'accepted'].includes(status) && (
                <Button variant="danger" className="w-full mt-2" onClick={handleCancel}>
                  Cancel Booking
                </Button>
              )}
            </Card>
