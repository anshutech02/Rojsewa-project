...
const ProviderDashboard = () => {
  ...
  const handleStatusUpdate = (bookingId, nextStatus) => {
    const confirmation = window.confirm(`Are you sure you want to update this booking status to: ${nextStatus.replace('_', ' ')}?`);
  ...
  const handleDeleteService = (serviceId) => {
    const confirmation = window.confirm('Are you sure you want to delete this service? This action cannot be undone.');
    if (!confirmation) return;
  ...
  const handleFormSubmit = (data) => {
    ...
    if (modalMode === 'create') {
      ...
      dispatch(updateService({ serviceId: selectedService._id, serviceData: data }))
        .unwrap()
        .then(() => {
  ...
  const pageLoading = (isServicesTab && servicesLoading && providerServices.length === 0) || 
    ...
    <DashboardLayout title="Provider Control Panel" sidebarLinks={providerLinks}>
      ...
      {statsLoading ? (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[1, 2, 3, 4, 5].map(n => <div key={n} className="glass h-24 shimmer rounded-xl" />)}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <Card hoverEffect={false} className="p-4 flex flex-col gap-1 relative overflow-hidden bg-gradient-to-br from-zinc-900/60 to-zinc-900/10">
            <span className="text-zinc-500 text-[10px] uppercase font-semibold tracking-wider">Total Bookings</span>
            <span className="text-2xl font-black text-zinc-100">{stats?.totalBookings || 0}</span>
            <Clock className="absolute right-3 bottom-3 text-zinc-800" size={32} />
          </Card>
          <Card hoverEffect={false} className="p-4 flex flex-col gap-1 relative overflow-hidden bg-gradient-to-br from-zinc-900/60 to-zinc-900/10">
            <span className="text-zinc-500 text-[10px] uppercase font-semibold tracking-wider">Active Bookings</span>
            <span className="text-2xl font-black text-indigo-400">{stats?.activeBookings || 0}</span>
            <Play className="absolute right-3 bottom-3 text-indigo-950/40" size={32} />
          </Card>
          <Card hoverEffect={false} className="p-4 flex flex-col gap-1 relative overflow-hidden bg-gradient-to-br from-zinc-900/60 to-zinc-900/10">
            <span className="text-zinc-500 text-[10px] uppercase font-semibold tracking-wider">Earnings</span>
            <span className="text-2xl font-black text-emerald-400">₹{stats?.totalEarnings || 0}</span>
            <DollarSign className="absolute right-3 bottom-3 text-emerald-950/40" size={32} />
          </Card>
          <Card hoverEffect={false} className="p-4 flex flex-col gap-1 relative overflow-hidden bg-gradient-to-br from-zinc-900/60 to-zinc-900/10">
            <span className="text-zinc-500 text-[10px] uppercase font-semibold tracking-wider">Rating</span>
            <span className="text-2xl font-black text-yellow-400">★ {stats?.rating?.toFixed(1) || '0.0'}</span>
            <Star className="absolute right-3 bottom-3 text-yellow-950/40" size={32} />
          </Card>
          <Card hoverEffect={false} className="p-4 flex flex-col gap-1 relative overflow-hidden bg-gradient-to-br from-zinc-900/60 to-zinc-900/10 col-span-2 md:col-span-1">
            <span className="text-zinc-500 text-[10px] uppercase font-semibold tracking-wider">Total Services</span>
            <span className="text-2xl font-black text-purple-400">{stats?.totalServices || 0}</span>
            <Sparkles className="absolute right-3 bottom-3 text-purple-950/40" size={32} />
          </Card>
        </div>
      )}
      ...
      {!isServicesTab ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          {bookings.length === 0 ? (
            <Card hoverEffect={false} className="p-8 text-center text-zinc-500 flex flex-col gap-2 items-center">
              <span className="text-3xl">📭</span>
              <p className="font-semibold text-zinc-400">No booking requests received yet.</p>
              <p className="text-xs max-w-sm">New client request notifications will show up here as soon as they are submitted.</p>
            </Card>
          ) : (
            <div className="flex flex-col gap-4">
              {bookings.map((booking) => (
                <Card key={booking._id} className="p-5 flex flex-col gap-4 border-l-2 border-l-zinc-700/80 hover:border-l-indigo-500 transition-all duration-300">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs text-zinc-400">
                        Customer: <span className="text-zinc-300 font-semibold">{booking.customer?.name}</span> | {booking.customer?.phone}
                      </span>
                    </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-bold tracking-wide">My Service Listing</h3>
            <Button variant="primary" size="sm" className="gap-1.5 shadow-indigo-600/10" onClick={handleOpenCreateModal}>
              <Plus size={14} /> Create Service
            </Button>
          </div>

          {providerServices.length === 0 ? (
            <Card hoverEffect={false} className="p-12 text-center text-zinc-500 flex flex-col gap-3 items-center bg-zinc-950/30">
              <span className="text-4xl">🛠️</span>
              <p className="font-bold text-zinc-300">No Services Listed Yet</p>
              <p className="text-xs max-w-sm leading-relaxed">List the services you specialize in, set your hourly rates or fixed charges, and define service times to start getting bookings.</p>
              <Button variant="outline" size="sm" className="mt-2" onClick={handleOpenCreateModal}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {providerServices.map((service) => (
                <Card key={service._id} className={`p-5 flex flex-col justify-between gap-4 border transition-all duration-300 ${
                  service.isActive ? 'border-zinc-800/80 hover:border-indigo-500/40' : 'border-dashed border-zinc-800 opacity-65 hover:opacity-90'
                }`}>
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest ${
                          service.isActive ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-400'
                        }`}>
                    ...
                    <div className="flex gap-4 items-center text-xs text-zinc-500 border-t border-zinc-900 pt-3 mt-1">
                      <div>
                        Rate: <span className="font-bold text-indigo-400">₹{service.price}</span>
                        <span className="text-[10px] text-zinc-500 capitalize"> ({service.priceType?.replace('_', ' ')})</span>
                      </div>
                      <div>
                        Duration: <span className="font-semibold text-zinc-300">{service.duration} mins</span>
                      </div>
                    </div>
                  </div>
